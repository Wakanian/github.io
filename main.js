...
// ★★★ ここから集計ロジックを修正 ★★★
    // stoneStateList の定義に合わせて、1=白、2=黒として集計する
    const whiteStonesNum = stoneStateList.filter(state => state === 1).length; // 1（白）の数を集計
    const blackStonesNum = stoneStateList.filter(state => state === 2).length; // 2（黒）の数を集計
    
    let winnerText = "";
